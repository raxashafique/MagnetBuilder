...
namespace CustomUtilities
{
	...
	public class CoroutineHandler : SingletonPersistent<CoroutineHandler>
	{
		...
		private static IEnumerator AfterWaitCoroutine(Action action, float seconds, bool realTime)
		{
		...
		public static Coroutine AfterWait(MonoBehaviour Obj, bool realTime = false,
		                                  params CoroutineSequence[] sequences)
		{
		...
		{
			foreach (var item in sequences) yield return item.Behaviour();
		}
		...
		private static IEnumerator AfterWaitCoroutine(Action action, Func<bool> condition)
		{
		...
		public static Coroutine WaitLoop(MonoBehaviour Obj, Action action, Func<bool> condition, float seconds = 0,
		                                 bool realTime = false)
		{
	...
	public static class CoroutineStatic
	{
		...
		public static Coroutine AfterWait(this MonoBehaviour Obj, bool realTime = false,
		                                  params CoroutineSequence[] sequences)
		{
	...
	public class UMUpdate
	{
		...
		{
			if (updateCoroutines.TryGetValue(delay, out var result))
			{
			...
			else
			{
				var data = new UpdateData
					{action = action, coroutine = mono.StartCoroutine(DoUpdateCoroutine(action, delay))};
		...
		{
			if (updateCoroutines.TryGetValue(delayRegistor, out var result))
			{
				result.action -= action;
		...
		private IEnumerator DoUpdateCoroutine(Action action, float delay)
		{
			while (true)
			{
				yield return new WaitForFixedUpdate();
	...
	public class TimeCounter : CoroutineDelay
	{
		...
		public TimeCounter(Action OnComplete, float m_seconds, int m_startFrom = 0, bool m_realTime = false) : base(
			OnComplete, m_seconds, m_realTime)
		{
			action = OnComplete;
			seconds = m_seconds;
			realTime = m_realTime;
		...
		public string Output(Format format = Format.minsecs)
		{
			...
			{
				var mints = (int) ((seconds - (incr + 1)) / 60);
				var sec = (int) ((seconds - (incr + 1)) % 60);
				resultString = mints.ToString("00") + " : " + sec.ToString("00");
			}
